...
# Alternate UK storefront search used by some regions:
...
def _search_nintendo_uncached(title: str, limit: int = 6) -> dict[str, Any]:
    ...
    html, status = fetch_html(url, timeout=10)
    if not html:
        # try .co.uk host
        alt = SEARCH_ALT.format(q=quote_plus(title))
        html, status = fetch_html(alt, timeout=10)
        if html:
            url = alt
            out["search_url"] = alt
    if not html:
    ...
    soup = soup_from(html)
    # Heuristic selectors — Nintendo markup changes often
    cards = soup.select(
...
def search_nintendo(title: str, limit: int = 6) -> dict[str, Any]:
    ...
    return cached(
        f"nintendo:search:v1:{title.lower()}",
